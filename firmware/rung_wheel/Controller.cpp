...
void Controller::setup()
{
  ...
  pattern_is_positive_ = true;
  increment_enabled_ = true;
  pattern_positive_count_ = 0;
  ...
  ModularDevice::Field & pattern_positive_count_lower_field = modular_server_.createField(constants::pattern_positive_count_lower_field_name,constants::pattern_positive_count_lower_default);
  pattern_positive_count_lower_field.setRange(constants::pattern_count_min,constants::pattern_count_max);

  ModularDevice::Field & pattern_positive_count_upper_field = modular_server_.createField(constants::pattern_positive_count_upper_field_name,constants::pattern_positive_count_upper_default);
  pattern_positive_count_upper_field.setRange(constants::pattern_count_min,constants::pattern_count_max);
...
void Controller::incrementPattern()
{
  ...
  {
    long pattern_positive_count_lower;
    modular_server_.getFieldValue(constants::pattern_positive_count_lower_field_name,
                                  pattern_positive_count_lower);
    long pattern_positive_count_upper;
    modular_server_.getFieldValue(constants::pattern_positive_count_upper_field_name,
                                  pattern_positive_count_upper);
    ...
    if (pattern_is_positive_ && (pattern_positive_inc_ == pattern_positive_count_))
    {
      if (pattern_negative_count > 0)
      {
        pattern_is_positive_ = false;
      }
      pattern_positive_inc_ = 0;
    }
    else if (!pattern_is_positive_ && (pattern_negative_inc_ == pattern_negative_count))
    {
      if (pattern_positive_count_upper > 0)
      {
        pattern_is_positive_ = true;
        pattern_positive_count_ = random(pattern_positive_count_lower,pattern_positive_count_upper);
        Serial << pattern_positive_count_ << "\n";
      }
      pattern_negative_inc_ = 0;
    }
    if (pattern_is_positive_ && (pattern_positive_inc_ < pattern_positive_count_))
